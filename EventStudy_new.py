...
class EventStudy():
    ...
    """
    def __init__(self, data, events, window, impose_mu=None):
        ...
        # rearrange to be unique (the best way with ordinal labels)
        events = \
        ...
        # pivot
        before, after, grouped_between_evts = self.pivot_for_event_study(
            data, events, window)

        # calculate variances
        if impose_mu is None:
            grouped_var = grouped_between_evts[data.name].groupby(
                grouped_between_evts["to_which_evt_belongs"]).var()
        else:
            grouped_var = \
                ((grouped_between_evts[data.name]-impose_mu)**2).groupby(
                grouped_between_evts["to_which_evt_belongs"]).mean()

        # retain integer index
        grouped_var.index = grouped_var.index.map(int)

        # variance without grouping by
        total_var = grouped_between_evts[data.name].var()

        # calculate means
        grouped_mu = grouped_between_evts[data.name].groupby(
            grouped_between_evts["to_which_evt_belongs"]).mean()

        # mean without grouping by
        total_mu = grouped_between_evts[data.name].mean()
    ...
    def pivot_for_event_study(data, events, window):
        ...
        data_to_pivot.loc[:,"evt_window"] = np.nan
        # for inference: from event window start till evt window end,
        #   including possible event window breaks inbetween
        data_to_pivot.loc[:,"evt_window_range"] = np.nan
        # introduce columns for pivoting
        data_to_pivot.loc[:,"evt_number"] = np.nan
        # introduce columns for standard deviations
        data_to_pivot.loc[:,"to_which_evt_belongs"] = 0
        ...
        for t, evt in events.iteritems():
            ...
            window_idx_for_var = np.arange((this_t+ta),(this_t+td+1))

            # put event identifier to be used for pivoting later
            data_to_pivot.ix[window_idx,"evt_number"] = evt
            # index with [ta:tb]+[tc:td]
            data_to_pivot.ix[window_idx,"evt_window"] = \
                    np.concatenate((np.arange(ta, tb+1),
                    np.arange(tc, td+1)))
            # index [ta:td]
            data_to_pivot.ix[window_idx_for_var,"evt_window_range"] = evt
            # for standard deviations
            data_to_pivot.ix[(this_t+ta):,"to_which_evt_belongs"] += 1

        # drop rows except those selected around event
        pivoted = data_to_pivot.dropna(subset=["evt_number"])

        # column to integers: need events and periods to be integers
        pivoted.loc[:,"evt_number"] = pivoted.loc[:,"evt_number"].map(int)
        pivoted.loc[:,"evt_window"] = pivoted.loc[:,"evt_window"].map(int)
        pivoted.loc[:,"to_which_evt_belongs"] = \
            pivoted.loc[:,"to_which_evt_belongs"].map(int)
        ...
        # for variance
        grouped_between_evts = \
            data_to_pivot.where(data_to_pivot["evt_window_range"].isnull())
        ...
        return before, after, grouped_between_evts
