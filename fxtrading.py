...
class FXTrading():
    ...
    def to_excel(self, filename):
        ...
        self.swap_points.loc["ask",:,:].to_excel(writer, sheet_name='swop_ask')
        self.swap_points.loc["bid",:,:].to_excel(writer, sheet_name='swop_bid')
...
if __name__ == "__main__":
    ...
    for h in range(10,16):
        ...
        for p in range(10,26):
            ...
            if add_usd:
                ...
                signals, signals_usd = signals.align(signals_usd,
                    axis=0, join="outer")
    ...
    with open(out_path + "ip_rx.p", mode="wb") as hangar:
        pickle.dump({"perf": nav_perf, "fcast": nav_fcast}, hangar)

    # with open(out_path + "ip_rx_wo_fomc.p", mode="rb") as hangar:
    ...
    #
    # fig, ax = plt.subplots()
    # data_flat.ffill().plot(ax=ax, color=new_gray)
    # data_flat.ffill().quantile([0.05,0.95], axis=1).T.plot(ax=ax,
    #     color='k', linewidth=1.5, linestyle="--")
    # data_flat.ffill().quantile(0.5, axis=1).plot(ax=ax,
    #     color='k', linewidth=1.5, linestyle="--")
    # ax.legend_.remove()
    #
    #
    ...
    # fig, ax = plt.subplots()
    # for h in data.items:
    #     data.loc[h,:,:].plot(ax=ax, color=new_gray)
    # ax.legend_.remove()
    #
    #
    ...
    #     data.loc[h,:,:].plot(
    #         ax=ax,
    #         color=matplotlib.colors.rgb2hex(cmap(h/15)[:3]))
    # ax.legend_.remove()
    #
    #
    # data = pd.concat([data.loc[:,:,p] for ])
    # fxtr.to_excel(out_path + "insights_swap.xlsx")
    #
    # ((data_merged_tz["tnswap_ask"]- data_merged_tz["tnswap_bid"]) > 0).sum()
